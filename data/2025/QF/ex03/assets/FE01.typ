...
#let fig() = {
  cetz.canvas({
    ...
    bouteille_vide(start_g, y_b)
    bouteille_vide(start_g + bw + b_gap, y_b)
    bouteille_vide(start_g + 2 * (bw + b_gap), y_b)
    bouteille_vide(start_g + 3 * (bw + b_gap), y_b)

    // droite : 1 bouteille demi
    let start_d = droite_c - bw / 2
    bouteille_demie(start_d, y_b)
  })
}